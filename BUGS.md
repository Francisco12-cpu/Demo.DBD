...
- **2ª rodada (mesmo dia) — "aliasing" nos desenhos, relatado pelo Francisco depois de testar no GitHub Pages:** achado (reproduzido localmente via screenshot) — não é falta de sprite nem bug de luz de novo, é moiré/tremor causado pelo zoom da câmera não ser múltiplo exato de pixel. `CAMERA_ZOOM_DESKTOP`/`MOBILE` (1.3/1.7) não são múltiplos de 1/16 — os tiles novos (base 16px, ver BUG-003) caem em fração de pixel (16×1.3=20.8px), o navegador tem que suavizar/arredondar tile a tile, gerando um padrão de xadrez fantasma visível na parede/chão. Além disso, o offset da câmera (`translate()`) seguia a posição CONTÍNUA (com casas decimais) do jogador sem arredondar, mudando por uma fração de pixel a cada frame — tremor constante ao andar. **Corrigido** com as "2 formas mais fáceis" (pedido do Francisco): (1) zoom ajustado pro múltiplo de 1/16 mais próximo (1.3→1.3125, 1.7→1.6875 — mudança de tamanho imperceptível, mas agora todo tile de 16/32/48/64px cai em pixel inteiro); (2) pixel-snap do offset da câmera (`Math.round()` antes do `translate()`) — a câmera só se move em saltos de 1px por frame, imperceptível a 60fps, mas elimina o tremor de sub-pixel. Testado: screenshot antes/depois mostrando o padrão de tijolo limpo (sem xadrez fantasma) na mesma cena; `npm test` sem regressão.
...
| 2026-08-16 | — | Francisco testou no GitHub Pages e relatou luz + "aliasing" nos desenhos. Achado (BUG-002, 2ª rodada): moiré causado pelo zoom da câmera (1.3/1.7) não ser múltiplo de pixel dos tiles de 16px, mais tremor de sub-pixel no offset da câmera a cada frame. Corrigido com zoom ajustado pra múltiplo de 1/16 (1.3125/1.6875) + pixel-snap do offset (`Math.round`). Testado com screenshot antes/depois; `npm test` sem regressão. |
